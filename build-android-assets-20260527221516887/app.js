...
const categoryKeywords = [
  ...
  { words: ["uber", "99", "posto", "combustível", "combustivel"], categoryId: "cat-transport" },
  { words: ["netflix", "spotify", "prime", "assinatura"], categoryId: "cat-bills" },
  { words: ["farmácia", "farmacia", "drogaria", "saúde", "saude"], categoryId: "cat-health" },
...
const defaultCategories = [
  ...
  { id: "cat-health", name: "Saúde", type: "expense", locked: true },
...
function updateBackButton() {
  ...
  const previousTitle = viewHistory.length ? getViewTitle(viewHistory[viewHistory.length - 1]) : "Início";
...
function syncOptionalForms(focusName = "") {
  const configs = {
    ...
    canBuy: { form: els.canBuyForm, button: els.toggleCanBuyForm, open: "Fechar", closed: canBuyAnalysis ? "Nova análise" : "Analisar compra" },
...
function syncBudgetFormMode() {
  ...
  if (submit) submit.textContent = editingBudgetId ? "Salvar alterações" : "Salvar meta";
}

function syncRecurringFormMode() {
  const submit = els.recurringForm.querySelector("button[type='submit']");
  if (submit) submit.textContent = editingRecurringId ? "Salvar alterações" : "Salvar recorrente";
...
function updateScreenTitle(viewName) {
  ...
  els.screenSubtitle.textContent = viewName === "dashboard" ? "Controle Financeiro" : "Finanças Pessoais";
...
function saveCardForm(event) {
  ...
  resetCardForm();
  render();
  showToast(cardId ? "Cartão atualizado." : "Cartão adicionado.");
}
...
function buildCanBuyAnalysis(form) {
  ...
  if (isCard && !card) {
    level = "danger";
    reasons.push("Cadastre um cartão antes de confirmar essa compra.");
  }
  if (projectedAfter < 0) {
    level = "danger";
  ...
  if (availableAfter < 0) {
    level = "danger";
    reasons.push(`Seu saldo disponível ficaria negativo em ${BRL.format(Math.abs(availableAfter))}.`);
  }
  if (cardAvailableAfter !== null && cardAvailableAfter < 0) {
    level = "danger";
    reasons.push(`Essa compra passaria ${BRL.format(Math.abs(cardAvailableAfter))} do limite do cartão.`);
  }
  if (budget && budgetPercentAfter > 100) {
    level = "danger";
    reasons.push(`${getCategoryName(input.categoryId)} passaria de 100% da meta do mês.`);
  }
  ...
  if (level !== "danger") {
    ...
    if (cardUsageAfter >= 80) {
      level = "warning";
      reasons.push(`O cartão ficaria com ${Math.round(cardUsageAfter)}% do limite comprometido.`);
    }
    if (projectedAfter < Math.max(summary.incomeTotal * 0.08, input.amount * 1.5)) {
      level = "warning";
      reasons.push("A folga do mês ficaria pequena depois dessa compra.");
    }
    if (!reasons.length) {
      reasons.push("Mesmo com essa compra, seu saldo previsto continua positivo e o mês segue controlado.");
    }
  }

  const title = level === "safe"
    ? "Essa compra cabe no seu mês!"
    : level === "warning"
  ...
  return {
    ...
    text: level === "safe"
      ...
      : level === "warning"
        ? "Ela ainda pode caber, mas reduz sua margem de segurança."
        : "O impacto deixa seu mês vulnerável.",
    reason: reasons[0],
    input,
    isCard,
    card,
...
function renderCanBuyResult(analysis) {
  ...
  const cardLimitHtml = analysis.isCard && analysis.card
    ? `<article><span>Limite após compra</span><strong>${BRL.format(analysis.metrics.cardAvailableAfter)}</strong></article>`
    : "";
  const summaryItems = [
    ["Compra", analysis.input.description],
    ["Valor", BRL.format(analysis.input.amount)],
    ["Data de lançamento", formatDate(analysis.input.date)],
    ["Categoria", getCategoryName(analysis.input.categoryId)],
    ["Pagamento", analysis.isCard ? "Cartão de crédito" : paymentMethodLabel(analysis.input.paymentMethod)],
  ];
  if (analysis.isCard && analysis.card) {
    summaryItems.push(["Cartão", `${analysis.card.name} final ${analysis.card.lastDigits || "----"}`]);
  ...
  const analyzedSummaryHtml = `
    <div class="can-buy-analysis-summary">
      <div>
        ...
        <span>Dados fixos desta simulação.</span>
      </div>
  ...
  els.canBuyResult.innerHTML = `
    ...
    <div class="can-buy-metrics">
      ${invoicePreviewHtml}
      <article><span>Saldo após compra</span><strong>${BRL.format(analysis.metrics.availableAfter)}</strong></article>
      <article><span>Saldo previsto no fim do mês</span><strong>${BRL.format(analysis.metrics.projectedAfter)}</strong></article>
      <article><span>Disponível por dia</span><strong>${BRL.format(analysis.metrics.dailyAfter)}</strong></article>
      ${cardLimitHtml}
    </div>
    ${budgetHtml}
    <div class="can-buy-schedule">
      <div>
        <strong>${analysis.isCard ? "Impacto por fatura" : "Impacto no mês"}</strong>
...
function confirmCanBuyPurchase() {
  ...
  showToast(isCard ? "Compra salva no cartão." : "Despesa salva.");
}
...
function saveBudget(event) {
  ...
  if (duplicate) {
    alert("Já existe uma meta para essa categoria.");
    return;
  }
  if (editingBudgetId) {
...
function toggleSmsPermission() {
  if (window.AndroidFinance?.openNotificationSettings) {
    ...
    persist();
    renderSmsList();
    showToast("Ative o acesso às notificações do app.");
    return;
  }
  ...
  window.FinanceNativeBridge = {
    receiveFinancialEvent(payload) {
      try {
        ...
        showToast("Não consegui ler a notificação recebida.");
      }
    },
  };
}
...
function normalizeNativeFinancialEvent(event = {}) {
  ...
  return {
    ...
    merchant: cleanText(event.merchant || event.establishment || "Lançamento detectado"),
...
function handleDocumentClick(event) {
  ...
  if (button.dataset.action === "open-card-detail") {
    openCardDetailFromDue(id);
    return;
  }
  if (button.dataset.action === "show-analysis-info") {
    showToast("Compara o mês atual com o anterior usando receitas, despesas, cartões, metas e contas.");
    return;
  }
  ...
  if (button.dataset.action === "delete-card") {
    ...
    if (state.purchases.some((purchase) => purchase.cardId === id)) {
      alert("Exclua as compras desse cartão antes de remover o cartão.");
      return;
    }
  ...
  if (button.dataset.action === "delete-category") {
    ...
    if (isCategoryInUse(id)) {
      alert("Essa categoria está em uso e não pode ser removida.");
      return;
    }
  ...
  if (button.dataset.action === "delete-recurring-occurrence") {
    ...
    showToast("Lançamento removido deste mês.");
  }
...
function openCardDetailFromDue(id) {
  selectedCardId = id;
  cardDetailHidden = false;
  switchView("cards");
  renderCards();
...
function renderFinancialCalendar(summary) {
  ...
  els.financialCalendar.innerHTML = `
    ...
    <div class="calendar-agenda">
      ${agenda.length ? agenda.map(calendarAgendaRow).join("") : emptyState("Sem vencimentos futuros neste mês.")}
    </div>
  `;
}
...
function buildFinancialCalendarEvents(summary) {
  ...
  const expenseEvents = [
    ...
    .map((item) => ({
      ...
      day: parseDateParts(item.date).day,
      title: item.description,
      meta: `${getCategoryName(item.categoryId)} • ${paymentMethodLabel(item.paymentMethod)}`,
      amount: item.amount,
    }));
...
function getFinancialStatus(summary) {
  ...
  if (summary.projectedBalance < 0) {
    return {
      ...
      text: `Seu saldo previsto fica ${BRL.format(Math.abs(summary.projectedBalance))} negativo. Revise cartões, recorrentes e despesas futuras.`,
  ...
  if (summary.dailyAvailable > 0 && summary.dailyAvailable < 50) {
    return {
      level: "warning",
      label: "Atenção no ritmo",
      title: "Gaste com mais critério",
      text: `Restam ${BRL.format(summary.dailyAvailable)} por dia por ${days} dias. Evite compras fora do plano até virar o mês.`,
  ...
  if (futureWeight >= 0.65) {
    return {
      ...
      text: `Você tem ${BRL.format(summary.futureCommitments)} em compromissos futuros. Confira vencimentos antes de novas compras.`,
  ...
  if (cardWeight >= 0.55) {
    return {
      level: "warning",
      label: "Cartão em destaque",
      title: "A fatura pesa no mês",
      text: `Cartões representam ${Math.round(cardWeight * 100)}% das despesas do mês. Acompanhe parcelas e limite disponível.`,
      action: "Ver cartões",
      target: "cards",
    };
  }
  ...
  return {
    ...
    title: "Você pode gastar com segurança",
    text: `Mantendo esse ritmo, o saldo previsto fecha em ${BRL.format(summary.projectedBalance)} e o limite diário fica em ${BRL.format(summary.dailyAvailable)}.`,
    action: "Ver análise",
    target: "analysis",
  };
}
  ...
  const rows = state.cards.map((card) => {
    ...
    return `
      <article class="invoice-row">
        ...
        <span>Limite disponível estimado: ${BRL.format(getCardAvailableLimit(card.id, selectedMonth))}</span>
      </article>
    `;
  });

  els.invoiceOverview.innerHTML = rows.length ? rows.join("") : emptyState("Nenhum cartão cadastrado.", "Novo cartão", "cards");
}

function renderCashBreakdown(summary) {
  const items = [
    ["Dinheiro disponível", summary.availableNow],
    ["Pix", summary.paymentTotals.Pix || 0],
    ["Débito", summary.paymentTotals.Debito || 0],
    ["Dinheiro", summary.paymentTotals.Dinheiro || 0],
    ["Cartão comprometido", summary.cardTotal],
...
function renderDueList(summary) {
  ...
  els.dueList.innerHTML = dues.length
    ...
    : emptyState("Sem vencimentos pendentes neste mês.", "Nova compra", "cards");
}
      ...
      <div class="due-top">
        <div class="card-title-block">
          ...
          <div>
            ...
            <span>${escapeHtml(getCategoryName(item.item.categoryId))} • ${escapeHtml(paymentMethodLabel(item.item.paymentMethod))}</span>
          </div>
        </div>
        <div class="due-amount-block">
          <strong class="amount-expense">${BRL.format(item.total)}</strong>
          <span class="due-badge ${escapeHtml(dueBadgeClass(days))}">${escapeHtml(dueBadgeText(days))}</span>
        </div>
      </div>
      <div class="recurring-due-footer">
...
function cardDueRow(item) {
  const invoiceUsage = item.card.limitTotal > 0 ? Math.min((item.total / item.card.limitTotal) * 100, 100) : 0;
  const days = daysBetween(todayInput(), item.dueDate);
  const mainInstallments = item.installments.slice(0, 3);
  const hiddenCount = Math.max(0, item.installments.length - mainInstallments.length);
  return `
    <article class="due-row card-due-card bank-due-${escapeHtml(item.card.bankId || "other")}">
      <div class="due-top">
        <div class="card-title-block">
          <div class="card-identity">${bankIcon(item.card.bankId, "small")}</div>
          <div>
      ...
      <div class="due-card-meta">
        <span>Fatura de ${escapeHtml(monthLabel(item.month))} • vence em ${escapeHtml(formatShortDate(item.dueDate))}</span>
        <strong>Uso: ${BRL.format(item.total)}</strong>
      </div>
      <div class="progress-track due-limit-track" aria-label="${Math.round(invoiceUsage)}% do limite usado">
        <div class="progress-fill" style="width:${invoiceUsage}%"></div>
      </div>
      <span class="due-limit-label">${Math.round(invoiceUsage)}% do limite usado</span>
      <div class="due-purchase-list">
        ${mainInstallments
          .map((installment) => `
            <div class="due-purchase-row">
              <div class="due-purchase-copy">
                <strong>${escapeHtml(installment.purchase.description)}</strong>
                <span>${escapeHtml(getBankName(item.card.bankId))} • ${escapeHtml(getCategoryName(installment.purchase.categoryId))}${installment.totalInstallments > 1 ? ` • ${installment.number}/${installment.totalInstallments}` : ""}</span>
              </div>
              <strong>${BRL.format(installment.amount)}</strong>
            </div>
          `).join("")}
        ${hiddenCount ? `<span class="due-more-items">+${hiddenCount} lançamentos</span>` : ""}
      </div>
      <button class="due-detail-link" type="button" data-action="open-card-detail" data-id="${escapeHtml(item.card.id)}">Ver detalhes <span aria-hidden="true">›</span></button>
    </article>
  `;
}
...
function renderCategorySummary(summary) {
  ...
  els.categorySummary.innerHTML = entries.length
    ...
    : emptyState("Sem gastos registrados neste mês.", "Nova despesa", "expenses");
}
...
function renderRecentActivity(summary) {
  const recent = [
    ...
    ...summary.cashExpenses.map((item) => ({
      ...
      kind: "expense",
      title: item.description,
      meta: `${formatDate(item.date)} • ${paymentMethodLabel(item.paymentMethod)}`,
      amount: item.amount,
      date: item.date,
    })),
  ...
  els.recentActivity.innerHTML = recent.length
    ...
    : emptyState("Sem lançamentos neste mês.", "Adicionar", "transactions");
}
  ...
  return [
    ...
    ...summary.cashExpenses.map((item) => {
      ...
      return {
        ...
        title: item.description,
        meta: `${formatDate(item.date)} • ${getCategoryName(item.categoryId)} • ${paymentMethodLabel(item.paymentMethod)}${isRecurringOccurrence ? ` • recorrente • ${recurringPaymentStatus(item).label}` : ""}`,
        amount: item.amount,
        kind: "expense",
...
function cardTransactionItem(item) {
  ...
  return {
    ...
    title: item.purchase.description,
    meta: launchMode
      ? `${formatDate(item.purchase.date)} • entra na fatura ${monthLabel(item.month)} • vence ${formatDate(item.dueDate)} • Cartão ${getCardName(item.purchase.cardId)} • ${getCategoryName(item.purchase.categoryId)}`
      : `Compra em ${formatDate(item.purchase.date)} • Fatura ${monthLabel(item.month)} • vence ${formatDate(item.dueDate)} • Cartão ${getCardName(item.purchase.cardId)} • ${getCategoryName(item.purchase.categoryId)}`,
    amount: item.amount,
    kind: "card",
    icon: "card",
...
function renderIncome() {
  ...
  els.incomeList.innerHTML = items.length
    ...
    : emptyState("Nenhuma receita cadastrada para este mês.", "Nova receita", "income");
}
  ...
  els.expenseList.innerHTML = items.length
    ? items.map((item) => {
        ...
        return moneyEntryRow(
          ...
          `${formatDate(item.date)} • ${getCategoryName(item.categoryId)} • ${paymentMethodLabel(item.paymentMethod)}${isRecurringOccurrence ? ` • recorrente • ${recurringPaymentStatus(item).label}` : ""}`,
          isRecurringOccurrence ? recurringOccurrenceKey(item) : item.id,
        );
      }).join("")
    : emptyState("Nenhuma despesa à vista cadastrada para este mês.", "Nova despesa", "expenses");
}
...
function renderCards() {
    ...
    rows.push(`
      <article class="card-row card-display card-summary card-tone-${(index % 6) + 1} bank-card-${escapeHtml(card.bankId || "other")} ${selectedCardId === card.id && !cardDetailHidden ? "selected-card" : ""}">
        ...
        <div class="card-summary-actions">
          ...
          <button class="card-icon-action edit" type="button" data-action="edit-card" data-id="${card.id}" aria-label="Editar cartão" title="Editar">${iconSvg("edit")}</button>
          <button class="card-icon-action danger" type="button" data-action="delete-card" data-id="${card.id}" aria-label="Excluir cartão" title="Excluir">${iconSvg("trash")}</button>
        </div>
  ...
  if (!isCreatingCard) {
    rows.push(`<button class="add-card-row" type="button" data-action="new-card">Adicionar cartão</button>`);
  }
  els.cardList.innerHTML = rows.length ? rows.join("") : emptyState("Nenhum cartão cadastrado.", "Novo cartão", "cards");
}
  ...
  return `
    <div class="inline-card-detail">
      ...
      <section class="card-detail-kpis">
        ...
        <article><span>Disponível</span><strong>${BRL.format(available)}</strong></article>
        <article><span>Próxima fatura</span><strong>${BRL.format(nextInvoice)}</strong></article>
...
function renderSmsList() {
  ...
  if (!state.settings.smsEnabled) {
    els.smsList.innerHTML = emptyState("A leitura automática está desativada.");
    return;
  }
  ...
  els.smsList.innerHTML = state.pendingSms.length ? state.pendingSms.map((item) => `
    <article class="sms-row">
      ...
      <div class="sms-detected">
        ...
        <div>
          <span>Tipo</span>
          <strong>${escapeHtml(item.type === "card" ? `Cartão final ${item.cardLastDigits || "----"}` : paymentMethodLabel(item.paymentMethod || "Pix"))}</strong>
        </div>
        <div>
...
function syncNotificationIndicator() {
  ...
  document.querySelectorAll(".bell-action").forEach((button) => {
    ...
    button.setAttribute("aria-label", count > 0 ? `Ver ${count} notificações pendentes` : "Ver mensagens");
  });
}
  ...
  return `
    <form class="card-row card-inline-form" data-card-form="${isEdit ? "edit" : "new"}">
      ...
      <div class="form-actions">
        ...
        <button class="primary-button" type="submit">${isEdit ? "Salvar alterações" : "Salvar cartão"}</button>
      </div>
    </form>
  `;
}
  ...
  els.categoryList.innerHTML = state.categories
    ...
    .map((category) => {
      ...
      <article class="category-list-row">
        ...
        <span class="pill ${category.type}">${category.locked ? "Padrão" : "Personalizada"}</span>
    ...
    ? state.recurringExpenses.map((item) => `
        <article class="recurring-row">
          <div>
            ...
            <span>${escapeHtml(getCategoryName(item.categoryId))} • vence dia ${item.dueDay} • ${escapeHtml(paymentMethodLabel(item.paymentMethod))}</span>
          </div>
          <strong>${BRL.format(item.amount)}</strong>
          <div class="card-actions">
...
function renderRecurringControl() {
  ...
  els.recurringControlList.innerHTML = items.length
    ...
    : emptyState("Nenhuma conta ou fatura ativa neste mês.");
}
  ...
  return `
    <article class="recurring-control-row ${escapeHtml(item.paymentStatus)}">
      ...
      <div class="recurring-control-main">
        <div>
          <strong>${escapeHtml(item.description)}</strong>
          <span>${escapeHtml(getCategoryName(item.categoryId))} • ${escapeHtml(paymentMethodLabel(item.paymentMethod))}</span>
          ${paidText}
        </div>
        <span class="status-pill ${escapeHtml(item.paymentStatus)}">${escapeHtml(status.label)}</span>
      </div>
      ...
      <div class="card-actions">
        ...
        <button class="delete-button icon-only" type="button" data-action="delete-recurring-occurrence" data-id="${key}" aria-label="Remover deste mês" title="Remover deste mês">${iconSvg("trash")}</button>
      </div>
    </article>
  `;
}
...
function renderAnalysis() {
  ...
  els.analysisHeader.innerHTML = `
    <div>
      ...
      <div class="analysis-month-line">
        <button class="icon-button" type="button" data-action="analysis-month-prev" aria-label="Mês anterior">&lsaquo;</button>
        ...
        <button class="icon-button" type="button" data-action="analysis-month-next" aria-label="Próximo mês">&rsaquo;</button>
      </div>
      <p>${hasPrevious ? `Comparado a ${escapeHtml(monthLabel(previousMonth))}` : "Ainda sem mês anterior suficiente para comparar."}</p>
    </div>
    <div class="analysis-header-actions">
      <button class="analysis-info-button" type="button" data-action="show-analysis-info" aria-label="Sobre a análise" title="A análise compara receitas, despesas, cartões, metas e contas recorrentes do mês selecionado.">${iconSvg("info")}</button>
    </div>
  `;
  ...
  els.analysisIncreasedCategories.innerHTML = hasPrevious
    ...
    : emptyState("Ainda não há dados suficientes para comparar aumentos.");
  els.analysisReducedCategories.innerHTML = hasPrevious
    ? renderAnalysisCategoryList(reduced, "down")
    : emptyState("Ainda não há dados suficientes para comparar reduções.");
  els.analysisHighlights.innerHTML = `
    ${renderAnalysisHighlight("good", "Melhor ponto do mês", bestPoint.title, bestPoint.text, "trophy")}
    ${renderAnalysisHighlight("warning", "Ponto de atenção", attentionPoint.title, attentionPoint.text, "warning")}
  `;
  els.analysisComparison.innerHTML = hasPrevious
    ? renderAnalysisComparison(currentTotals, previousTotals)
    : emptyState("Continue lançando suas movimentações para gerar uma comparação mais completa.");
}
...
function renderAnalysisEvolutionChart(series) {
  const lines = [
    ...
    { key: "card", label: "Cartão", values: series.card },
  ...
  return `
    <div class="analysis-chart-shell">
      <svg viewBox="0 0 ${viewWidth} ${viewHeight}" role="img" aria-label="Evolução de receitas, despesas, cartão e saldo">
...
function renderAnalysisCategoryList(items, direction) {
  if (!items.length) {
    return emptyState(direction === "up" ? "Nenhuma categoria aumentou neste mês." : "Nenhuma categoria reduziu neste mês.");
  }
...
function getAnalysisScore(current, previous) {
  if (!hasAnalysisData(current)) {
    return {
      value: 0,
      status: "Sem dados",
      text: "Cadastre receitas e despesas para gerar uma leitura real do mês.",
    };
  }
  const currentTotals = getAnalysisTotals(current);
  const previousTotals = getAnalysisTotals(previous);
  ...
  const status = value <= 40 ? "Crítico" : value <= 60 ? "Atenção" : value <= 80 ? "Controlado" : "Muito bom";
  const text = value <= 40
    ? "O mês precisa de cortes ou reforço de receita."
    : value <= 60
      ? "Ha pontos importantes para acompanhar de perto."
      : value <= 80
        ? "O mês está controlado, com alguns cuidados."
        : "Seu mês está saudável e com boa margem.";
...
function getAnalysisBestPoint(current, previous, reduced, score) {
  if (!hasAnalysisData(current)) {
    return {
      title: "Sua análise está pronta para receber dados.",
      text: "Assim que você lançar movimentações, o app destaca o melhor ponto do mês.",
    };
  }
  ...
  if (currentSavings > previousSavings && hasAnalysisData(previous)) {
    return {
      title: "Você economizou mais que no mês anterior.",
      text: `A economia subiu ${BRL.format(currentSavings - previousSavings)} e fortaleceu seu fechamento.`,
    };
  }
  if (reduced.length) {
    return {
      title: `${getCategoryName(reduced[0].categoryId)} caiu neste mês.`,
      text: `Você reduziu ${BRL.format(Math.abs(reduced[0].diff))} nessa categoria.`,
    };
  }
  if (score.value >= 81) {
    return {
      title: "Gastos dentro do orçamento!",
      text: "Parabéns. Seu mês está com boa margem de segurança.",
    };
  }
  return {
    title: current.projectedBalance >= 0 ? "Saldo previsto positivo." : "Você está acompanhando o risco.",
...
function getAnalysisAttentionPoint(current, previous, increased) {
  if (!hasAnalysisData(current)) {
    return {
      ...
      text: "Inclua receitas, despesas ou compras para revelar pontos de atenção.",
    };
  }
  if (current.projectedBalance < 0) {
    return {
      title: "Risco de saldo negativo.",
      text: `Faltam ${BRL.format(Math.abs(current.projectedBalance))} para fechar o mês no azul.`,
    };
  }
  if (current.recurringLateTotal > 0) {
    return {
      title: "Existem contas atrasadas.",
      text: `${BRL.format(current.recurringLateTotal)} em recorrentes atrasados ainda precisam de atenção.`,
    };
  }
  ...
  if (overBudget) {
    return {
      ...
      text: `Você já usou ${Math.round(overBudget.percent)}% da meta da categoria.`,
    };
  }
  ...
  if (increased.length) {
    return {
      ...
      text: `Alta de ${BRL.format(increased[0].diff)} em relação ao mês anterior.`,
    };
  }

  if (previous.cardTotal > 0 && current.cardTotal > previous.cardTotal) {
    return {
      title: "Cartão subiu neste mês.",
      text: `A fatura aumentou ${BRL.format(current.cardTotal - previous.cardTotal)} frente ao mês anterior.`,
    };
  }

  return {
    title: "Nada crítico por enquanto.",
...
function renderAnalysisComparison(current, previous) {
  return [
    ...
    comparisonRow("Cartão", current.card, previous.card, false),
  ...
  return `
    <article class="analysis-comparison-row">
      <div>
        ...
        <span>Mês anterior: ${BRL.format(previous)}</span>
      </div>
      <small class="${escapeHtml(change.tone)}">${escapeHtml(change.label)}</small>
    </article>
  `;
...
function renderInstallmentPreview() {
  if (!state.cards.length) {
    els.installmentPreview.textContent = "Cadastre um cartão para simular as parcelas.";
    return;
  }
...
function getInvoiceMonthForTransactionDate(dateString, card) {
  ...
  // A fatura é identificada pelo mês de vencimento; a compra entra na primeira fatura cujo fechamento ainda não passou.
...
function getCardName(id) {
  return state.cards.find((card) => card.id === id)?.name || "Cartão removido";
}
...
function buildSmartAlerts(summary) {
  ...
  if (summary.recurringLateTotal > 0) {
    addSmartAlert(
      ...
      `Há ${BRL.format(summary.recurringLateTotal)} em contas fixas vencidas e ainda pendentes.`,
      "recurringControl",
    );
    ...
    addSmartAlert(
      ...
      `Ainda falta pagar ${BRL.format(summary.recurringPendingTotal)} em recorrentes deste mês.`,
      "recurringControl",
    );
  }
  addSmartAlert(
    ...
    "Disponível por dia",
    `Você ainda tem ${BRL.format(summary.dailyAvailable)} por dia por ${remainingDays} dias até o fim do mês.`,
    "dashboard",
  );
  ...
  state.budgets.forEach((budget) => {
    ...
    if (percent >= 0.8) {
      addSmartAlert(
        ...
        `Você já usou ${Math.round(percent * 100)}% da meta. Restam ${BRL.format(Math.max(0, budget.limit - spent))}.`,
        "categories",
      );
    }
  });
  ...
  Object.entries(summary.categoryTotals).forEach(([categoryId, current]) => {
    ...
    if (previousAmount >= 50 && diff >= 50 && current > previousAmount * 1.25) {
      addSmartAlert(
        ...
        "warning",
        68,
        `${getCategoryName(categoryId)} acima da média`,
        `Você gastou ${BRL.format(diff)} a mais que no mês passado. Veja se ainda cabe no plano.`,
        "categories",
      );
    }
  });
  ...
  if (delivery >= 100 && !previousDelivery) {
    addSmartAlert(alerts, "warning", 66, "Delivery entrou forte", `Já foram ${BRL.format(delivery)} em delivery/restaurantes neste mês.`, "transactions");
  } else if (previousDelivery >= 30 && delivery - previousDelivery >= 40 && delivery > previousDelivery * 1.25) {
    addSmartAlert(
      alerts,
      "warning",
      72,
      "Delivery subiu neste mês",
      `Você gastou ${BRL.format(delivery - previousDelivery)} a mais que no mês anterior.`,
      "transactions",
    );
  }
  ...
  state.cards.forEach((card) => {
    ...
    if (previous > 0 && diff >= 50 && current > previous * 1.3) {
      addSmartAlert(
        ...
        `Ela está ${Math.round(((current / previous) - 1) * 100)}% maior que no mês anterior. Confira compras recentes.`,
        "cards",
      );
    }
  });
...
function normalizeState(input) {
  ...
  defaultCategories.forEach((category) => {
    const existing = normalized.categories.find((item) => item.id === category.id);
    if (existing) Object.assign(existing, { name: category.name, type: category.type, locked: category.locked });
    else normalized.categories.push(category);
  });
...
function createSeedData() {
  ...
  return normalizeState({
    ...
    incomes: [
      { id: createId(), description: "Salário", amount: 6200, date: dateInMonth(month, 5), categoryId: "cat-salary", status: "received" },
    ...
    expenses: [
      ...
      { id: createId(), description: "Mercado do mês", amount: 438.9, date: dateInMonth(month, 9), categoryId: "cat-market", paymentMethod: "Debito" },
    ...
    purchases: [
      ...
      { id: createId(), description: "Farmácia", amount: 126.7, date: dateInMonth(month, 8), categoryId: "cat-health", cardId: cardOne, installments: 1 },
    ...
    skippedRecurring: [],
    pendingSms: [
      parseFinancialSms("Compra aprovada no cartão final 1234 no valor de R$ 187,40 em MERCADO EXTRA.", dateInMonth(month, 7), "nubank"),
      parseFinancialSms("Pix enviado para JOAO SILVA no valor de R$ 80,00.", dateInMonth(month, 8), "inter"),
      parseFinancialSms("Compra aprovada no cartão final 5678 no valor de R$ 28,50 em UBER.", dateInMonth(month, 8), "bb"),
      parseFinancialSms("Compra aprovada no cartão final 1234 no valor de R$ 39,90 em NETFLIX.COM.", dateInMonth(month, 10), "nubank"),
    ],
